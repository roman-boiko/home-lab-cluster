...
- `clusters/lab/gitops/platform/`: platform components managed by Argo CD: `argocd`, `authentik`, `cert-manager`, `cilium`, `gateway`, `longhorn`, and `sealed-secrets`.
...
The root app is `home-lab-cluster`. Child apps self-manage Argo CD, Authentik, Cilium, cert-manager, Longhorn, Sealed Secrets, and the public/private Gateway split. Avoid direct `kubectl apply`, manual Helm installs, and node-local edits except for diagnostics or recovery.
